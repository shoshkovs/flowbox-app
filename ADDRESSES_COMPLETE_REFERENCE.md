...
4. **Парсинг:** Автоматически парсит `house` из `street`, если `house` не указан
5. **Дедупликация:** Проверяет дубликаты перед добавлением
...
5. Добавление в `savedAddresses`
6. Вызов `saveUserData()` → POST `/api/user-data`
7. Бэкенд: `saveUserAddresses(user.id, addresses)`
8. Бэкенд: INSERT нового адреса в БД
9. Обновление UI через `loadSavedAddresses()`
...
6. Обновление в `savedAddresses[index]`
7. Вызов `saveUserData()` → POST `/api/user-data`
8. Бэкенд: `saveUserAddresses(user.id, addresses)`
9. Бэкенд: UPDATE существующего адреса в БД
10. Обновление UI через `loadSavedAddresses()`
...
6. Бэкенд: DELETE адреса из БД (если его нет в новом списке)
7. Принудительное обновление `localStorage`
8. Обновление UI через `loadSavedAddresses()`
...
6. Принудительное обновление `localStorage` с пустым массивом
7. При перезагрузке: GET `/api/user-data/:userId` → возвращает `addresses: []`
8. Фронтенд: использует пустой массив, адреса не возвращаются
