-- Упрощенное расширение схемы БД (без отдельных входов для админов)
-- Добавляет функционал курьеров, зон доставки и промокодов
...
BEGIN
    IF OLD.status IS DISTINCT FROM NEW.status THEN
        ...
        VALUES (NEW.id, NEW.status, 'admin', NULL);
    END IF;
    RETURN NEW;
END;
$$ language 'plpgsql';
